...
var jjrmodelidlist =new Array(); 	 //用于存放从数据库取出的所有节假日的id
var jjrmodeltimelist =new Array();	 //用于存放从数据库取出的所有节假日的time
var jjrmodelztlist =new Array(); 	 //用于存放从数据库取出的所有节假日的状态
...
//创建年份下拉,并给对应事件(定义了yearselect并赋值,且添加事件，选择则对应的table日期也将改变,且已选中日期会跳到当前选择月的日期，然后给右边明细栏赋值)
function createSelectYear(){
	...
	var yearSelect= withID("aboluo-yearSelect");   //获取年份下拉框对象
	var Nowtime=new Date();					
	var currYear=Nowtime.getFullYear();			   //获取当前年份
	for(var i=0;i<=10;i++){						   //下拉框显示自2015年起，50年
		yearSelect.options.add(new Option((i+2018)+"年",i+2018));
		if(currYear==i+2018){
			yearSelect.options[i].selected=true;  //下拉框默认选中当前年份 
		}
	}
	yearSelect.onchange=function(e){
		...
		getjjrszModelByYear(withID("aboluo-yearSelect").value);		//获取某年，节假日信息	
...
function getjjrszModelByYear(year){
	var contextPath = withID("contextPath").value;									
	//清除原有数据
	jjrmodelidlist =new Array(); 	 //用于存放从数据库取出的所有节假日的id    ['1','2'];
	jjrmodeltimelist =new Array();	 //用于存放从数据库取出的所有节假日的time  ['2018-06-30 00:00:00','2018-06-19 00:00:00'];   //这里时间的格式为yyyy-MM-dd HH:mm:ss
	jjrmodelztlist =new Array(); 	 //用于存放从数据库取出的所有节假日的状态          ['0','1'];   //所有节假日的状态      0为上班，1为休息
	
	$.ajax({
		type:"post",
		url:contextPath+'/micromvc/holidays/getHolidayInfoByYear',
		async:false,
		data:{"year":year},
		dataType: 'json',
		success:function(json){ 
			if(json!=null && json!="" && parseInt(json.holidaySize)>0){
				var dataSize = parseInt(json.holidaySize);
				var data=json.data;
				for(var i=0;i< dataSize;i++){
					jjrmodelidlist.push(data[i].id);
					jjrmodeltimelist.push(data[i].cld_date);
					jjrmodelztlist.push(data[i].is_rest_day);
				}
			}
		}
	});
...
//设置日期状态的方式为 单选按钮	
	var szrqinputradio = newElement("input");
	szrqinputradio.type="radio";
	szrqinputradio.name="holidaystatus";
	szrqinputradio.id="status0";
	szrqinputradio.value="0";
	var stauts0 = createDateRedio('input','holidaystatus', 'radio', '0','holidaystatus0');	
	szrq.appendChild(stauts0);
	var spanWork=newElement('span');
	spanWork.innerHTML="上班&nbsp;&nbsp;";
	szrq.appendChild(spanWork);
	var stauts1 = createDateRedio('input','holidaystatus', 'radio', '1','holidaystatus1');	
	szrq.appendChild(stauts1);
	var spanRest=newElement('span');
	spanRest.innerHTML="休息";
	szrq.appendChild(spanRest);
	//设置右侧日期
	setaclass(year,yue,day);
	//给右侧单选按钮设置是否选中
	var nowDateStatus = getAclickDom().getAttribute("jjrzt");  //获取当天状态  (可能为null,null为上班日)
	if(nowDateStatus=="0" || nowDateStatus==null || nowDateStatus==""){
		withID("holidaystatus0").checked=true;
	}else if(nowDateStatus=="1"){
		withID("holidaystatus1").checked=true;
	}
	
}

/*创建假日单选按钮控件*/
function createDateRedio(tagName,name,type,value,id)
{
    var element = null;
    try{
    ...
    if (!element){
        ...
        element.setAttribute("onchange","javascript:aboluoSetrq(this);");
   }
   return element;
} 
	...
	for(var i=0;i<arr.length;i++){
		...
		for(var n=0;n<jjrmodelidlist.length;n++){
			if(formatByDate(jjrmodeltimelist[n])==formatByDate(date)){
				if(jjrmodelztlist[n]==0){ //0上班
				...
				}else if(jjrmodelztlist[n]==1){ //1休息
					var span=newElement('span');
					...
					span.innerHTML="休";
					arr[i].appendChild(span);
				}else if(jjrmodelztlist[n]==-1){ // 这里为了保证操作过的节假日的唯一性,不给样式只设置a的ztid
					arr[i].setAttribute("ztid",jjrmodelidlist[n]);
					arr[i].setAttribute("jjrzt",jjrmodelztlist[n]);
				}
			}
...
//单选按钮
	var dataStatus = object.value;  //要设置的当前日期状态
//下拉框
//	var szjjr=withClass("aboluo-ssjjr");    //要设置的当前日期状态
//	var dataStatus=szjjr.childNodes[1];
	$.ajax({
		   type:"post",
		   url:contextPath+'/micromvc/holidays/setHolidayStatus',
		   data:{"date":date,"dataStatus":dataStatus,"dataId":ztid,"dataOldStatus":jjrzt}, //这里用ajax可以改变状态
		   async:false,
		   dataType: 'json',
		   success:function(json){
			   if(json.operateStatus == "true"){
