...
<head>
	...
	<script src="<%=path%>/nh-micro-js/js/system/account-manage.js"></script>
	<script>
		$(function () {
            initAccountInfo();
        })
        function renderBusinessButton(cellValue,options,rowObject){
            var button = "";
            if(rowObject.statusVal === "240"){//交租待确认
                button += "<shiro:hasPermission name='rent_info_detail'>" +
                    "<a  class='btn btn-primary btn-xs' href='javascript:rentDetail(\""+rowObject.id+"\",\""+rowObject.statusVal+"\");'>交租详情</a>"+
                    "</shiro:hasPermission>";
            }
            button += "<shiro:hasPermission name='rent_check'>" +
                "<a  class='btn btn-primary btn-xs' href='javascript:detail(\""+rowObject.id+"\",\""+rowObject.statusVal+"\");'>查看</a>"+
                "</shiro:hasPermission>";
            return button;
        }
	</script>
