...
List nhRoleList=(List)session.getAttribute("nhRoleList");
boolean adminFlag=false;
if(nhRoleList!=null){
	if(nhRoleList.contains("role_system_admin")){
		adminFlag=true;
	}
}
%>
...
<script type="text/javascript" charset="UTF-8">
	...
	$(function() {
		tree = $('#tree-sys').tree({
			url : '<%=path%>/micropage/nh-micro-jsp/layout/west_menu.jsp',
			animate : false,
	...
	var setting = {
			...
			data : {
				...
				simpleData : { //简单的数据源，一般开发中都是从数据库里读取，API有介绍，这里只是本地的                           
					enable : true,
					idKey : "menu_id", //id和pid，这里不用多说了吧，树的目录级别  
    ...
    function zTreeOnClick(event, treeId, treeNode) {
        ...
        var node = nodes[0];
		if(node.menu_url && node.menu_url != ''){
			var href;
			href = "<%=path %>/"+node.menu_url;
			$centerFrame.attr('src',href);

		}
    };	
	function initBizTree() {
		var url = "<%=path%>/micromvc/uc/menuList/getInfoListAll4UserId";
	$.ajax({
			url:url,
			type:'post',
			dataType:'json',
