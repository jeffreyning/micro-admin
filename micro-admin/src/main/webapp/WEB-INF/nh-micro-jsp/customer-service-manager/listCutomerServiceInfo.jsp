...
<head>
	...
	<script>
        ...
        function renderBusinessButton(cellValue,options,rowObject){
            var button = "";
            if(rowObject.statusVal === "190"){//合同签约后审核
                button += "<shiro:hasPermission name='contract_audit'>" +
                    "<a  class='btn btn-primary btn-xs' href='javascript:contract(\""+rowObject.id+"\",\""+rowObject.statusVal+"\");'>合同审核</a>"+
                    "</shiro:hasPermission>";
            }
            button += "<shiro:hasPermission name='audit_detail'>" +
                "<a  class='btn btn-primary btn-xs' href='javascript:detail(\""+rowObject.id+"\",\""+rowObject.statusVal+"\");'>详情</a>"+
                "</shiro:hasPermission>";
            return button;
        }
	</script>
